...
public class CopTestleri : IDisposable
{
    ...
    public void Sil_DosyayiCopeTasir_ESKI_YERINDEN_KALKAR()
    {
        ...
        IslemRaporu rapor = Cop.Sil(_kok, yol);

        Assert.True(rapor.Oldu);
        Assert.False(File.Exists(yol));

        IReadOnlyList<CopOgesi> liste = Cop.Listele(_kok);
        Assert.Single(liste);
    ...
    public void GeriYukle_DOSYAYI_ICERIGIYLE_GERI_GETIRIR()
    {
        ...
        IslemRaporu rapor = Cop.GeriYukle(_kok, Cop.Listele(_kok)[0]);

        Assert.True(rapor.Oldu);
        Assert.Equal("onemli veri", File.ReadAllText(yol));
        Assert.Empty(Cop.Listele(_kok));
    }
    ...
    public void Sil_KlasoruIcindekilerleTasir_GeriYukleyince_ICI_YERINDE()
    {
        ...
        Assert.True(Cop.Sil(_kok, Yol("montaj")).Oldu);
        Assert.False(Directory.Exists(Yol("montaj")));

        CopOgesi oge = Cop.Listele(_kok)[0];
        Assert.True(oge.KlasorMu);
        Assert.True(Cop.GeriYukle(_kok, oge).Oldu);
    ...
    public void GeriYukle_ESKI_KLASOR_SILINMISSE_YENIDEN_ACAR()
    {
        ...
        Cop.Sil(_kok, Yol("alt", "Parca.SLDPRT"));
    ...
    public void Sil_AYNI_ADLI_IKI_DOSYA_BIRBIRINI_EZMEZ()
    {
        ...
        Assert.True(Cop.Sil(_kok, Yol("a", "P.SLDPRT")).Oldu);
        Assert.True(Cop.Sil(_kok, Yol("b", "P.SLDPRT")).Oldu);

        Assert.Equal(2, Cop.Listele(_kok).Count);

        foreach (CopOgesi oge in Cop.Listele(_kok))
        {
            Assert.True(Cop.GeriYukle(_kok, oge).Oldu);
        }
    ...
    {
        Cop.Sil(_kok, DosyaKoy("P.SLDPRT"));

        Assert.True(Cop.KaliciSil(_kok, Cop.Listele(_kok)[0]).Oldu);
        Assert.Empty(Cop.Listele(_kok));
    }
    ...
    public void Listele_DISKTE_OLMAYAN_KAYIT_GOSTERILMEZ()
    {
        Cop.Sil(_kok, DosyaKoy("P.SLDPRT"));
        CopOgesi oge = Cop.Listele(_kok)[0];
        Directory.Delete(WindowsYolu.Birlestir(Cop.Yolu(_kok), oge.No), recursive: true);
    ...
    public void Sil_COP_KLASORUNUN_KENDISI_COPE_ATILAMAZ()
    {
        ...
        Assert.Equal(IslemSonucu.KendiAltina, Cop.Sil(_kok, Cop.Yolu(_kok)).Sonuc);
    }
