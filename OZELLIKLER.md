...
| **Yeni versiyon oluştur…** | `Ctrl+Shift+U` | O anki içeriği **arşivler** (ilk çağrı `v0`'ı yaratır); kısa bir not sorar. **Yalnız o dosya** arşivlenir — montaj ve teknik resim dahil. Dosya yerinde kalır. Panelde: **satırın** dosyasına açılır | Tek dosya seçili değilken · SOLIDWORKS dosyası değilken |
...
- **Çift tık** → o versiyonu **açar** (salt-okunur). Her tür açılır; arşivde asıl dosya gerçek adıyla durur, hiçbir yere geçici dosya çıkmaz.
  **AMA montaj/teknik resimde parçalar yanında DEĞİL** (versiyon yalnız o dosya): SOLIDWORKS onları yanında bulamaz ve **bugünkü** parçalarla açar — ya da hiç açamaz. Durum çubuğu bunu **söyler**: *"parçaları yanında değil — BUGÜNKÜ parçalarla açılır"*. Eski (31.08–01.09) çocuklu arşivlerde o cümle çıkmaz, çünkü orada parçalar gerçekten yanında.
  **"Yazım erişimi için kullanılamıyor — salt okunur açılsın mı?" kutusu NORMALDİR** (Erkan, 01.09.2026): arşiv kopyaları diske **salt-okunur** yazılıyor, bilerek. Eski bir versiyonu açıp üstünde çalışıp kazayla kaydedersen geçmişi ezerdin; salt-okunur tam da onu engelliyor. **"Salt Okunur Olarak Aç"** de, açılır. Kutuyu bir daha görmek istemiyorsan **"Bir daha gösterme"**yi işaretle — SOLIDWORKS cevabı hatırlar.
- **VERSİYON = YALNIZ O DOSYANIN KOPYASI** (01.09.2026, Erkan'ın kararı). Kutu tek cümle: *"X şimdiki hâliyle arşivlenecek."* Başka hiçbir dosya kopyalanmaz — montaj kendi parçalarını, teknik resim kendi modelini arşive **almaz**.
  Bir tur önce alıyordu (*"kendi kendine yeten versiyon"*) ve bedeli görüldü: tek bir teknik resim **5**, in-context yapılmış tek bir parça **162** dosya sürüklüyordu.
  **Bedeli bilerek kabul edildi:** arşivdeki montajı/teknik resmi açmak artık bugünkü parçaları getirir (yukarıda yazıyor). *"Bu versiyona dön"* bundan **etkilenmez** — o dosyayı kendi yoluna geri yazar, parçaları zaten yerindedir.
- **Eski arşivler bozulmadı** (hem 31.08 öncesi tek dosyalık düzen, hem 31.08–01.09 arası çocuklu düzen): listelenir, açılır, dönülür, silinir. Çocuklu olanlarda *"neler geri yazılsın"* bölümü çalışmaya devam eder.

- **Versiyon satırına sağ tık** → dosya işlemleri uygulanmaz (gri + sebep): F2/Sil arşive gitseydi kayıt ile kopya eşleşmesi kırılırdı. Menünün üstünde o satırda tuşların **ne yaptığı** yazar (`Enter`: bu versiyona dön · `F2`: not · `Delete`: versiyonu sil) — menüdeki "Sil `Delete`" etiketi başka bir işi anlatıyordu.
- **Bu versiyona dön** → sekmede satırı seç, **Enter**. Kutu **iki yönü birden** gösterir:
  ...
  - **Aşağı (neler geri yazılsın)** → **yalnız eski (çocuklu) arşivlerde**: o versiyonun yanındaki dosyalardan hangileri geri yazılsın, sen seçersin. Yeni versiyonlarda tek dosya olduğu için bu bölüm hiç çizilmez. Bugünküyle **farklı** olanlar işaretli gelir; değişmemişe dokunulmaz. SOLIDWORKS'te açık ya da bugün yerinde olmayan dosya işaretlenemez, sebebi yanında yazar. Bu versiyonun başka dosyası yoksa o bölüm hiç çizilmez.
